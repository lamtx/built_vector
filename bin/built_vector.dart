...
void main(List<String> args) async {
  ...
  if (results["input"] == null) {
    return print("an input file path must be provided");
  }
  if (results["output"] == null) {
    return print("an ouput file path must be provided");
  }

  // Parsing
  final input = File(results["input"] as String);
  final content = await input.readAsString();
  final assetsParser = AssetsParser();
  final assets = assetsParser.parse(content);

  // Generating
  final output = File(results["output"] as String);
